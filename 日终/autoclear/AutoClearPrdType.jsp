<%--  
    @system  综合理财平台
    @version 4.0.0.1
    @lastModiDate 2013-12-12
    @describe   
    V3.0.0.1 20131212-01 yuanhb       分业务清算     M201312170010
    V3.0.0.2 20140325-01 yuanhb       现场测试问题修复  M201404030002
...
<%
  ...
  String subFlowFlag = (String)request.getAttribute("SubFlowFlag");//0-行内理财业务清算界面,某个含虚拟节点的业务清算界面
  //added by yuanhb 20140325 begin
  String prefix = IConstant.MARKET_CODE_PREFIX + prdType;
  //added by yuanhb 20140325 end
%>
...
function onActiveClick(){
	var func = function(operFlag){
		//added by yuanhb 20140325 begin
		//var param={nodeId:clickedNodeId,taCode:"000000",ExceuteMode:"2",operFlag:operFlag};
		var param={nodeId:clickedNodeId,taCode:"<%=prefix%>" + "000000",ExceuteMode:"2",operFlag:operFlag};
		//added by yuanhb 20140325 end
		try {
	...
	//added by yuanhb 20140325 begin
	//var checkParam = {taskCode:clickedNodeId,taCode:"000000"};
	var checkParam = {taskCode:clickedNodeId,taCode:"<%=prefix%>" + "000000"};
	//added by yuanhb 20140325 end
...
function onStepClick(){
	var func = function(operFlag){
		//added by yuanhb 20140325 begin
		//var param={nodeId:clickedNodeId,taCode:"000000",ExceuteMode:"1",operFlag:operFlag};
		var param={nodeId:clickedNodeId,taCode:"<%=prefix%>" + "000000",ExceuteMode:"1",operFlag:operFlag};
		//added by yuanhb 20140325 end
		try {
			sendAndToHost(actionUrl,param);
		} catch (e) {
...
function onDetailClick(){
	...
	var param={ExceuteMode:"X",taskCode:taskCode,marketCode:marketCode,parentTaskCode:parentTaskCode};
	try {
...
function onDownClick(){
	...
	var param={ExceuteMode:"D",taskCode:taskCode,marketCode:marketCode};
	try {
...
function leftClick(obj){
	...
	var func = function(operFlag){
		...
		//added by yuanhb 20140325 begin
		//var checkParam = {taskCode:obj.id,taCode:"000000"};
		var checkParam = {taskCode:obj.id,taCode:"<%=prefix%>" + "000000"};
		//added by yuanhb 20140325 end
		showMessage(func,checkUrl,checkParam);
...
function timingSendUpdate(){
	<%if(null!=ds && ds.findColumn("id")>0){%>
		if(canSend){
			canSend = false;
			var param={flowType:"3",ExceuteMode:"T", SubFlowFlag:"<%=subFlowFlag%>", PrdType:"<%=prdType%>"};
			try {
